...
class VoteTest < ActiveSupport::TestCase
  ...
  test "deletion of upvote should remove from poster's points" do
  ...
  test "deletion of downvote should remove from poster's points" do
    old_points = @second_user.points
    @downvote.destroy
    @second_user.reload
    assert_equal old_points + 1, @second_user.points
  ...
  test "upvote to downvote should decrement post's user by two" do
    old_points = @first_user.points
    @upvote.update_attribute :polarity, -1
    @first_user.reload
  ...
  test "downvote to upvote should increment post's user by two" do
    old_points = @second_user.points
    @downvote.update_attribute :polarity, 1
    @second_user.reload
