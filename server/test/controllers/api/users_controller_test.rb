...
class Api::UsersControllerTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @vote = Vote.create(votable: @post2, user_id: @user.id, polarity: 1)
    @downvote = Vote.create(votable: @post, user_id: @user2.id, polarity: -1)
  end
  ...
  test "confirm method should set the user to confirmed" do
    ...
    post "/api/users/" + @unconfirmed_user.username + "/confirm/" + @unconfirmed_user.confirmation_code, params: { confirmation_code: @unconfirmed_user.confirmation_code }
  ...
  test "confirm method should deny incorrect confirmation codes" do
    assert_not @unconfirmed_user.confirmed
    post "/api/users/" + @unconfirmed_user.username + "/confirm/incorrect_code", params: { confirmation_code: "incorrect_code" }
    @unconfirmed_user.reload
    assert_not @unconfirmed_user.confirmed
  end
  ...
  test "should return correct upvotes" do
    get "/api/users/" + @user.username + "/upvotes"
    assert_response 200
    body = JSON.parse(response.body)
  ...
  test "should return correct downvotes" do
    get "/api/users/" + @user2.username + "/downvotes"
    assert_response 200
    body = JSON.parse(response.body)
  ...
  test "should update bio with valid input" do
    ...
    patch "/api/users/" + @user.username, params: { user: update_params }, headers: { "Authorization" => @user.auth_token }
    @user.reload
    assert_response 200
  ...
  test "should update password with valid input" do
    ...
    patch "/api/users/" + @user.username + "/update_password", params: { user: password_params }, headers: { "Authorization" => @user.auth_token }
    @user.reload
